...
int main(void)
{
    ...
    //PORTA的输出反转寄存器地址=基地址+偏移量
    volatile uint32_t *portA_PTOR = gpioA_ptr + 3;

    //（1.5.3）设置PORTA5引脚为GPIO引脚，即令相应引脚控制寄存器的10-8位
    //(MUX)字段为001
    ...
    *portA_PDO |= (1 << 5);
    ...
    //（1.5.5）通过PORTA的方向寄存器，定义PORTA5引脚输出
    *portA_PDDR |= (1 << 5);

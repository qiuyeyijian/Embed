...
//=====================================================================
//函数返回：无
//参数说明：x1,y1:左上角点坐标(240*320)
//         x2,y2:右下角点坐标(240*320)
//         color:颜色（使用本文件后画笔颜色宏定义）
...
//Bit5: 0:字节内象素数据从高位到低位排列，
//      1:字节内象素数据从低位到高位排列。
//Bit4: 0:WORD类型高低位字节顺序与PC相同，
//      1:WORD类型高低位字节顺序与PC相反。
//Bit3~2: 保留。
//Bit1~0: [00]水平扫描，[01]垂直扫描，
//        [10]数据水平,字节垂直，[11]数据垂直,字节水平。
//gray: 灰度值
// 1:单色，2:四灰，4:十六灰，8:256色，12:4096色，
// 16:16位彩色，24:24位彩色，32:32位彩色。
//w: 图像的宽度。
//h: 图像的高度。
//is565: 在4096色模式下为0表示使用[16bits(WORD)]格式，
//此时图像数据中每个WORD表示一个像素；
//为1表示使用[12bits(连续字节流)]格式，
//此时连续排列的每12Bits代表一个像素。
//在16位彩色模式下为0表示R G B颜色分量所占用的位数都为5Bits，
//为1表示R G B颜色分量所占用的位数,分别为5Bits,6Bits,5Bits。
//在18位彩色模式下为0表示"6Bits in Low Byte"，
//为1表示"6Bits in High Byte"。
...
// 汉字表： 金葫芦【】
//=====================================================================
#define ChineseCharacterLength (sizeof(GB_16)/sizeof(typFNT_GB16))   
											  //记录汉字的个数
const static typFNT_GB16  GB_16[] =        // 数据表
{
	...
	"张", 0x80,0x00,0x9F,0x10,0x90,0x10,0x90,0x08,
		0x90,0x04,0x9E,0x02,0x82,0x00,0xE2,0x7F,
		0x82,0x02,0x9E,0x04,0x90,0x04,0x90,0x08,
		0x90,0x10,0x90,0x22,0x8A,0x41,0x84,0x00,

	"龙", 0x20,0x04,0x20,0x08,0x20,0x08,0x20,0x00,
		0xFF,0x7F,0x20,0x01,0x20,0x11,0x20,0x11,
		0x20,0x09,0x10,0x05,0x10,0x03,0x08,0x41,
		0x88,0x41,0x44,0x41,0x22,0x7E,0x01,0x00,

	"威", 0x00,0x14,0x00,0x24,0x00,0x04,0xFC,0x7F,
		0x04,0x04,0x04,0x04,0xFC,0x27,0x44,0x24,
		0x44,0x24,0xFC,0x15,0x24,0x15,0x14,0x09,
		0xA4,0x48,0x42,0x54,0xA2,0x62,0x19,0x41,

	"秋", 0x10,0x02,0x38,0x02,0x0F,0x02,0x08,0x02,
		0x88,0x22,0xBF,0x22,0x88,0x12,0x4C,0x0A,
		0x1C,0x02,0x2A,0x05,0x2A,0x05,0x09,0x09,
		0x88,0x08,0x88,0x10,0x48,0x20,0x28,0x40,

	"叶", 0x00,0x04,0x00,0x04,0x1E,0x04,0x12,0x04,
		0x12,0x04,0x12,0x04,0xD2,0x7F,0x12,0x04,
		0x12,0x04,0x12,0x04,0x1E,0x04,0x12,0x04,
		0x00,0x04,0x00,0x04,0x00,0x04,0x00,0x04,

	"依", 0x10,0x01,0x10,0x02,0x10,0x02,0xE8,0x7F,
		0x08,0x01,0x0C,0x01,0x8C,0x02,0x8A,0x22,
		0xC9,0x12,0xA8,0x0C,0x98,0x04,0x88,0x08,
		0x88,0x10,0x88,0x22,0x88,0x41,0x88,0x00,

	"剑", 0x10,0x20,0x10,0x20,0x28,0x20,0x48,0x24,
		0x84,0x24,0x02,0x25,0x7D,0x24,0x00,0x24,
		0x88,0x24,0x90,0x24,0x92,0x24,0x44,0x24,
		0x44,0x20,0xE0,0x21,0x1E,0x28,0x04,0x10,

		// "潘", 0x00,0x1E,0xE4,0x03,0x48,0x12,0x88,0x0A,
		// 	0xE1,0x3F,0x82,0x0A,0x42,0x12,0x28,0x60,
		// 	0xC8,0x1F,0x44,0x12,0x47,0x12,0xC4,0x1F,
		// 	0x44,0x12,0x44,0x12,0xC4,0x1F,0x40,0x10
};

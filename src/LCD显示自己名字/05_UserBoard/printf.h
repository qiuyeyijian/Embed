...
其中格式控制字符串用于指定输出格式。
格式控制串可由格式字符串和非格式字符串两种组成。格式字符串是以%开头
的字符串，在%后面跟有各种格式字符，以说明输出数据的类型、形式、长度、小数位数等。
如：
   ” %d”表示按十进制整型输出；
   ” %ld”表示按十进制长整型输出；
   ” %f”表示浮点型输出。
   ...
   ” %c”表示按字符型输出。
   ” \n”表示换行符等。
       非格式字符串原样输出，在显示中起提示作用。输出表列中给出了各个输出项,
       要求格式字符串和各输出项在数量和类型上应该一一对应。
C.1.1 格式字符串
       格式字符串的一般形式为：[标志][输出最小宽度][精度][长度]类型，
其中方括号[]中的项为可选项。以下说明各项的意义。
1. 类型
...
4. 精度
        精度格式符以“.”开头，后跟十进制整数。本项的意义是：如果输出
 数字，则表示小数的位数；如果输出的是字符，则表示输出字符的个数；若实
 际位数大于所定义的精度数，则截去超过的部分。
5. 长度
...
浮点数精度说明：
             经过测试发现浮点数的精度为小数点后5位。
                浮点数的最后两位存在随机误差这与KEA128芯片的内核有关,
经过测试发现与其同内核的KL25存在同样的问题。
 *
 */
 ...
 /*
     ...
     //浮点数类型数据输出测试
     printf("浮点型数据输出测试:\n");
     //浮点数有效数字是7位，结果f=3.14159297
     printf("浮点数f=%f\n", f);
     //输出6列，小数点后4位，结果 f=3.1416
     printf("浮点数fhavassda  =  %6.4f\n", f);
     //输出长浮点数 x=0.12345678912345678
     printf("double型数x=%lf\n", x);
     //输出18列，小数点后15位，x=0.123456789123456

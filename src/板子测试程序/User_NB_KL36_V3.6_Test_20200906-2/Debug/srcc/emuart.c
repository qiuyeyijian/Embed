...
uint16_t emuart_frame(uint8_t ch, uint8_t *data)
{
	...
	//（4）如果接收到的数据达到一帧长度（帧长度=有效数据长度+8），
    //	   进行CRC校验，如果校验通过将进行数据的处理。
	if((length-1) != 0 && (index-1) >= length+7)
	{
		...
		if(data[index-3]!=emuartFrameTail[0] 
		    || data[index-2]!= emuartFrameTail[1]//δ?????β
			||	CRC_check[0] != data[index-5] 
			|| CRC_check[1] != data[index-4])//CRC???????
		{
			//出错返回
			index = 1;
			length = 1;
		...
		//（4.3）将前4个字节移出buf数组，令buf数组全为有效数据，
        //		 并返回有效数据
...
uint16_t useremuart_frame(uint8_t ch, uint8_t *data)
{
	...
	if((length-1) != 0 && (index-1) >= length+7)
	{
		...
		if((length-1) == 45)
		{
		    ...
		    {
		        flash_read_physical((uint8_t*)imei,(MCU_SECTOR_NUM-1)*
		                         MCU_SECTORSIZE+MCU_FLASH_ADDR_START,16);
